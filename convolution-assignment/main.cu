...
int main(int argc, char* argv[])
{
  ...
  printf("Allocating device variables..."); fflush(stdout);
    ...
    float sizef = sizeof(float) * FILTER_SIZE * FILTER_SIZE;
    cudaMemcpyToSymbol(M_c, M_h.elements, sizef);
  ...
  /*  printf("Mask Values: "); 
    for(int i = 0; i < M_h.width * M_h.height; i++){
    printf("%f ", M_h.elements[i]);
    }*/
    ...
    dim3 dim_grid((imageWidth/TILE_SIZE)+ 1,(imageHeight/TILE_SIZE) + 1, 1);
    ...
    unsigned grid = imageWidth/TILE_SIZE + 1 ;
    unsigned gridH = imageHeight/TILE_SIZE + 1;
    printf("Grid Width size: %u\n ", grid); fflush(stdout);
    printf("Grid Height size: %u\n ", gridH); fflush(stdout);
